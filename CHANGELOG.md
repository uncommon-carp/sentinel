...
- **`inventory.excessive_data_exposure` finding (API3)**: When an OpenAPI spec is loaded, the inventory suite now walks each GET operation's declared 200 response schema for sensitive-looking property names (`apiKey`, `password`, `secret`, `token`, `ssn`, `internal`, `role`, `isAdmin`, or `format: password`) and confirms with a live read that the field is actually present and non-null — a schema match alone doesn't flag, since a documented-but-unused property isn't evidence of exposure. GET operations with zero or one integer/number path parameter are probed (candidate ids `1`–`5`, same approach as `auth.bola_object_access`); more than one path parameter is skipped. Medium severity; evidence names the leaked field, never its value. Registered in `FINDINGS.md`.
- **`auth.mass_assignment_accepted` finding (API3)**: Opt-in via `auth.massAssignmentProbe` (default `false` — mutating, same precedent as `inventory.ssrfActiveProbe`). Discovers writable object endpoints (PATCH/PUT paths keyed by a single integer/number path id, same discovery shape as the BOLA probe) and sends a canary field absent from the operation's documented request-body schema, then re-reads the resource to check whether it persisted. A persisted, undocumented field is flagged high severity. Each configured identity is only probed against resources reachable through its own session — never another identity's object, since a write has side effects a read doesn't. Works with a single identity; with Tier-2 identities configured, each is probed independently. Registered in `FINDINGS.md`.
- **`businessFlow.sensitivePaths` config (API6 groundwork)**: A new config block (sibling to `inventory`/`auth`) where method+path pairs or operationIds can be declared as sensitive business flows (checkout, coupon redemption, password reset, ...). Black-box path/name guessing can't reliably identify these, so Sentinel requires an explicit declaration — the same "the spec alone can't reveal this" precedent already established by `auth.probePaths`/`auth.identities`. Pure config plumbing; empty (default) means no business-flow check runs. Documented in the README's Configuration section.
- **`ratelimit.sensitive_flow_unthrottled` finding (API6)**: The ratelimit suite gained a third phase that runs only when `businessFlow.sensitivePaths` is non-empty — the existing burst-probe mechanism (phase 2) retargeted at each declared endpoint instead of the first selected one. An unthrottled declared-sensitive endpoint gets a distinct, higher-severity finding (high, vs. the generic `ratelimit.no_429_on_burst`'s medium) since unrestricted automation on a named business flow has direct fraud/business impact. A throttled-but-missing-`Retry-After` result on a declared flow still emits the existing `ratelimit.missing_retry_after`, not a new ID. Registered in `FINDINGS.md`.
